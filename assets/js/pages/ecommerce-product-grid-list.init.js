        ...
        0 < i
          ? ((n = '<div class="ribbon ribbon-danger">Sale</div>'),
            '<h5 class="fs-lg mb-3">$' +
              ...
              e[c].price +
              "</small></h5>")
          : ((n = ""), '<h5 class="fs-lg mb-3">' + e[c].price + "</h5>")),
      (document.getElementById("product-grid").innerHTML +=
...
function paginationEvents() {
  ...
  prevButton.addEventListener("click", function () {
    ...
    nextButton.addEventListener("click", function () {
      currentPage < e() && loadProductList(productListData, ++currentPage);
    });
      ...
      (a = t.getAttribute("data-remove-id")),
        document
          ...
          .addEventListener("click", function () {
            ...
            (t = a),
              loadProductList(
                (productListData = productListData.filter(function (e) {
                  return e.id != t;
                })),
                currentPage
              ),
  ...
  e &&
    ...
    e.noUiSlider.on("update", function (e, t) {
      var a = productListData,
        r = (t ? (c.value = e[t]) : (i.value = e[t]), c.value.substr(2)),
        n = i.value.substr(2);
      loadProductList(
        a.filter(function (e) {
          if (e.price)
...
searchProductList.addEventListener("keyup", function () {
  ...
  (t = e),
    loadProductList(
      productListData.filter(function (e) {
        return (
          -1 !== e.product[0].title.toLowerCase().indexOf(t.toLowerCase()) ||
  ...
  Array.from(document.querySelectorAll(".filter-list a")).forEach(function (a) {
    ...
    loadProductList(
      (filterDataAll =
        a.querySelector(".form-check-input").checked && 0 == r
          ? e.filter(function (e) {
              if (e.discount)
                return (e = e.discount.split("%")), parseFloat(e[0]) < 10;
            })
          : a.querySelector(".form-check-input").checked && 0 < arraylist.length
          ? ((t = Math.min.apply(Math, arraylist)),
            e.filter(function (e) {
              if (e.discount)
                return (e = e.discount.split("%")), parseFloat(e[0]) >= t;
            }))
          : productListData),
      currentPage
    );
